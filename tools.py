"""
Tools for the Flood Risk Assistant agent.

Design choice: flood risk is judged RELATIVE TO EACH DISTRICT'S OWN historical
discharge, not a single fixed number across the whole country. A river like the
Jamuna naturally carries far more water than a smaller river, so one flat
threshold would either flag the big rivers as "always risky" or the small ones
as "never risky." Instead, each district's own trailing-365-day 75th/90th
percentile is computed in BigQuery and used as that district's personal
baseline.
"""

from google.cloud import bigquery

PROJECT_ID_PLACEHOLDER = "YOUR_GCP_PROJECT_ID"  # set via configure_project()
_project_id = PROJECT_ID_PLACEHOLDER


def configure_project(project_id: str) -> None:
    """Call once at startup with your real GCP project ID."""
    global _project_id
    _project_id = project_id


def check_flood_risk(district: str) -> dict:
    """Check current flood risk for a Bangladeshi district.

    Compares the district's most recent river discharge reading against that
    SAME district's own trailing-365-day 75th and 90th percentile discharge,
    so a "normal" flow on a small river isn't confused with a "high" flow on
    a much bigger one.

    Args:
        district: District name, e.g. "Sirajganj", "Dhaka", "Sylhet".

    Returns:
        A dict with: district, latest_date, latest_discharge_m3s, risk_level
        ("normal", "elevated", or "high"), p75_m3s, p90_m3s. If the district
        has no data yet, risk_level is "unknown" and a `note` explains why.
    """
    client = bigquery.Client(project=_project_id)

    query = f"""
        WITH baseline AS (
          SELECT
            district,
            date,
            river_discharge_m3s,
            PERCENTILE_CONT(river_discharge_m3s, 0.75)
              OVER (PARTITION BY district) AS p75_m3s,
            PERCENTILE_CONT(river_discharge_m3s, 0.90)
              OVER (PARTITION BY district) AS p90_m3s
          FROM `{_project_id}.flood_risk.river_discharge_daily`
          WHERE district = @district
            AND date >= DATE_SUB(CURRENT_DATE(), INTERVAL 365 DAY)
        )
        SELECT *
        FROM baseline
        ORDER BY date DESC
        LIMIT 1
    """
    job_config = bigquery.QueryJobConfig(
        query_parameters=[bigquery.ScalarQueryParameter("district", "STRING", district)]
    )
    rows = list(client.query(query, job_config=job_config).result())

    if not rows:
        return {
            "district": district,
            "risk_level": "unknown",
            "note": (
                f"No discharge data found for '{district}'. Either it hasn't "
                "been ingested yet (see data_ingestion/districts.py) or the "
                "name doesn't match — call list_covered_districts to check."
            ),
        }

    row = rows[0]
    latest = row["river_discharge_m3s"]
    p75, p90 = row["p75_m3s"], row["p90_m3s"]

    if latest >= p90:
        risk_level = "high"
    elif latest >= p75:
        risk_level = "elevated"
    else:
        risk_level = "normal"

    return {
        "district": district,
        "latest_date": row["date"].isoformat(),
        "latest_discharge_m3s": round(latest, 1),
        "risk_level": risk_level,
        "p75_m3s": round(p75, 1),
        "p90_m3s": round(p90, 1),
    }


def list_covered_districts() -> dict:
    """List districts currently covered by the flood dataset.

    Returns:
        dict with a "districts" list. Use this before check_flood_risk if
        you're unsure whether a district is covered yet.
    """
    client = bigquery.Client(project=_project_id)
    query = f"""
        SELECT DISTINCT district, division
        FROM `{_project_id}.flood_risk.river_discharge_daily`
        ORDER BY district
    """
    rows = list(client.query(query).result())
    return {"districts": [{"district": r["district"], "division": r["division"]} for r in rows]}
